  ...
  (util/defer [resolve reject]
    (let [^object req (.open js/indexedDB project version)]
      ...
      (set! (.-onsuccess req)
            (fn success []
              (set! conn req.result)
              (set! (.-onabort conn) (util/cb on-abort))
              (set! (.-onclose conn) (util/cb on-close))
              (set! (.-onerror conn) (util/cb on-error))
              (set! (.-onversionchange conn) (util/cb on-version-change))
              (resolve))))))

(defn init []
  (doto ^js/Node (.-body js/document)
    (.addEventListener "dragenter" (util/cb on-drag-enter))
    (.addEventListener "dragend"   (util/cb on-drag-end))
    (.addEventListener "dragover"  (util/cb on-drag-over))
    (.addEventListener "drop"      (util/cb on-drop)))
